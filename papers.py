...
class PaperTree(TMTree):
    ...
    _authors:
        The name(s) of the author(s) for this paper
    _doi:
        The doi for this paper
    ...
    with open(DATA_FILE, newline='') as csvfile:
        ...
        if by_year:
            ...
            for row in filereader:
                ...
                categories.insert(0, row[2])
                categories.append(row[1])
                nested_dict_helper(return_dict, categories, row)
        else:
            return_dict = {}
            for row in filereader:
                categories = row[3].split(': ')
...
def nested_dict_helper(d: Dict, keys: list, value: list) -> None:
    """Generates a nested dictionary with each key's value being another
    dictionary if the key is a category, or a list of information about the
    paper if the key is the title of a paper."""
    for key in keys[:-1]:
        d = d.setdefault(key, {})
    d[keys[-1]] = value
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    for key in nested_dict:
        if isinstance(nested_dict[key], list):
            row = nested_dict[key]
            paper = PaperTree(row[1], [], row[0], row[4], int(row[5]))
            local_subtrees.append(paper)
        else:
            lower_subtrees = _build_tree_from_dict(nested_dict[key])
            category = PaperTree(key, lower_subtrees)
            local_subtrees.append(category)
    return local_subtrees
...
if __name__ == '__main__':
    ...
    # print(helper_dictionary(['FLP', 'other', 'language agnostic approaches'],
    #                           'Separation of Introductory Programming '))
    # d = _load_papers_to_dict(False)
    # a = {'CS1': {'Paper1': ['a', 1, 1, 1, 1, 1],
    #              'Paper2': ['b', 2, 2, 2, 2, 2]}}
    # print(_build_tree_from_dict(a))
    # print(_load_papers_to_dict(True)['2003']['CA']['pair programming'])
